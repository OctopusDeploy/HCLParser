...
public class HclParser
{
    ...
    /// </summary>
    public static readonly Regex LineEndNormalize = new("\r\n?|\n");

    /// <summary>
    ///     A regex that matches the numbers that can be assigned to a property
    ///     matches floats, scientific and hex numbers
    /// </summary>
    public static readonly Regex NumberRegex = new(@"-?(0x)?\d+(e(\+|-)?\d+)?(\.\d*(e(\+|-)?\d+)?)?");

    /// <summary>
    ///     A regex that matches true and false
    /// </summary>
    public static readonly Regex TrueFalse = new(@"true|false", RegexOptions.IgnoreCase);
